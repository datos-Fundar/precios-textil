...
# Cargar URL 
pais_elegido <- 'Chile'
urls <- 'https://www.mercadolibre.cl/'
...
for(i in 1:length(productos$categoria)){
  ...
  # Ir a la web 
  remote_driver$navigate('https://www.mercadolibre.cl/')
  ...
  filtro_tiendas_elegido <- remote_driver$findElements(using='css',value='[title="Quitar el filtro de Tiendas oficiales Solo tiendas oficiales"]')
  if(length(tienda_oficial) > 0){
    remote_driver$executeScript("arguments[0].scrollIntoView(true);", list(tienda_oficial[[1]]))
    tienda_oficial[[1]]$clickElement()
  } else if(length(filtro_tiendas_elegido) == 0){
    tienda_oficial[[1]]$clickElement()
  }
...
# Guardar csv 
write_csv(data_final,file.path(outstub,paste0('05_chile_scraper_manual_tiendas_oficiales ',Sys.Date(),'.csv')))
saveRDS(data_final,file.path(outstub,paste0('05_chile_scraper_manual_tiendas_oficiales ',Sys.Date(),'.rds')))
