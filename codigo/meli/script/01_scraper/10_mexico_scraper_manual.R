...
# Cargar URL 
pais_elegido <- 'México'
urls <- 'https://www.mercadolibre.com.mx/'
...
for(i in 1:length(productos$categoria)){
  ...
  # Ir a la web 
  remote_driver$navigate('https://www.mercadolibre.com.mx/')
  ...
  for(j in 1:(productos_tmp)){
    ...
    for(k in 1:length(box_data)){
      nombre <- box_data[[k]] %>%
        html_elements(css='[class="ui-search-item__group__element ui-search-link"]') %>%
        html_text()
...
# Guardar csv 
write_csv(data_final,file.path(outstub,paste0('10_mexico_scraper_manual ',Sys.Date(),'.csv')))
saveRDS(data_final,file.path(outstub,paste0('10_mexico_scraper_manual ',Sys.Date(),'.rds')))