...
for(i in 1:length(productos$categoria)){
  ...
  # Ir a la web 
  remote_driver$navigate('https://www.mercadolivre.com.br/')
  ...
  tienda_oficial <- remote_driver$findElements(using='css',value='[aria-label="Somente lojas oficiais"]')
  ...
  for(j in 1:(productos_tmp)){
    ...
    for(k in 1:length(box_data)){
      ...
      # Tipo de envio
      envio <- box_data[[k]] %>%
        html_elements(css='[class="ui-pb-container"]') %>%
        html_text()
...
# Guardar csv 
write_csv(data_final,file.path(outstub,paste0('03_brasil_scraper_manual_tiendas_oficiales ',Sys.Date(),'.csv')))
saveRDS(data_final,file.path(outstub,paste0('03_brasil_scraper_manual_tiendas_oficiales ',Sys.Date(),'.rds')))