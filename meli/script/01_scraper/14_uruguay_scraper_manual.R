...
# Cargar URL 
pais_elegido <- 'Uruguay'
urls <- 'https://www.mercadolibre.com.uy/'
...
productos <- productos %>% 
  mutate(busqueda_producto = if_else(busqueda_producto == 'Tobilleras','Medias tobilleras',busqueda_producto))

# Armar scraper 
data_final <- tibble()
i<- 1
for(i in 39:length(productos$categoria)){
  
  # Ir a la web 
  remote_driver$navigate('https://www.mercadolibre.com.uy/')
  ...
  for(j in 1:(productos_tmp)){
    ...
    for(k in 1:length(box_data)){
      ...
      # Juntar todo
      tmp_final <- data.frame(nombre,precio_venta,moneda_venta,precio_lista,cuotas,cuotas_normales,highlight,envio,descuento,url_producto,todo)
...
# Guardar csv 
write_csv(data_final,file.path(outstub,paste0('14_uruguay_scraper_manual ',Sys.Date(),'.csv')))
saveRDS(data_final,file.path(outstub,paste0('14_uruguay_scraper_manual ',Sys.Date(),'.rds')))
